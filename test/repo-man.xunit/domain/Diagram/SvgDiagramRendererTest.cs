...
namespace repo_man.xunit.domain.Diagram
{
    public class SvgDiagramRendererTest : TestBase
    {
        ...
        public async Task Composes_Svg()
        {
            ...
            var dummyLegendDictionary = new Dictionary<string, string> { { "cs", "red" } };
            ...
            _mocker.GetMock<ISvgChartDataWriter>().Setup(x => x.WriteChartData(tree))
                .Returns(chartData);
            _mocker.GetMock<FileExtensionLegendDataBuilder>().Setup(x => x.BuildLegendOptions(tree))
                .Returns(dummyLegendDictionary);
            _mocker.GetMock<ISvgLegendDataWriter>().Setup(x => x.WriteLegendData(dummyLegendDictionary, point))
                .Returns(legendData);
        ...
        public async Task Uses_Path_From_Config_And_Default_File_Name()
        {
            ...
            _mocker.GetMock<ISvgLegendDataWriter>().Setup(x => x.WriteLegendData(It.IsAny<Dictionary<string,string>>(), It.IsAny<Point>()))
